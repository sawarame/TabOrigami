...
### 主な機能
- **AI整理:** おまかせ、タスク集中、仕事とプライベート、断捨離の4つのスタイルでタブを分類。
...
- **プロジェクトのビルド:**
  ...
  npm run build
  ```
  実行後、`package/js/` フォルダにJSファイルが出力され、ルートに `package.zip` が生成されます。
- **アイコンの生成:**
  ```bash
  node generate_icons.js
  ```
  `src/icon.svg` を元に、拡張機能に必要な各サイズのPNG画像を `package/icons/` に生成します。
